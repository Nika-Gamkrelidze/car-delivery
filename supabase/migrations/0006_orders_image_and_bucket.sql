...
create policy if not exists "Public read access for order-images" on storage.objects
  for select using ( bucket_id = 'order-images' );

-- Allow authenticated users to upload to the bucket
create policy if not exists "Authenticated upload for order-images" on storage.objects
...
create policy if not exists "Authenticated update own objects order-images" on storage.objects
  for update using (
    bucket_id = 'order-images' and auth.role() = 'authenticated' and owner = auth.uid()
  );
create policy if not exists "Authenticated delete own objects order-images" on storage.objects
  for delete using (
    bucket_id = 'order-images' and auth.role() = 'authenticated' and owner = auth.uid()
  );
